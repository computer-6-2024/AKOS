...
class ServerListManager:
    ...
    def _execute_query(self, sql: str, params: tuple = None, fetch_results: bool = False) -> Union[list, int, None]:
        """
        执行SQL查询或命令。包含自动重连逻辑。
        """
        # 为适配python3.8 windows7系统而改Union[list, int, None]
...
class IntranetServerManager:
    ...
    def _execute_query(self, sql: str, params: tuple = None, fetch_results: bool = False) -> Union[list, int, None]:
        ...
        Returns:
            Union[list, int, None]: 如果fetch_results为True，返回查询结果列表；为适配python3.8 windows7系统而改
...
class AKOS:
    ...
    def kill_ice(self):
        tk.messagebox.showerror("错误","windows7不受支持，请使用windows10以上版本安装好软件后到windows7电脑上进入BIOS进PE系统爆破冰点")
